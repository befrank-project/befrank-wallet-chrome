// Parse BeFrank URIs:
function parseBeFrankURI(url) {
  var parsed = { url: url };
  var r = /^befrank:([a-zA-Z0-9]{95})(?:\?(.*))?$/;
...
// Parses BeFrank URI and puts into wallet's Send tab and opens wallet:
function openBeFrankWalletCLIsend(href) {
  var parsed_href = parseBeFrankURI(href);
  var href_json = {
  ...
  var request = {greeting: "BeFrank befrank-wallet-rpc Payment Request", href: href_json};
  chrome.runtime.sendMessage(request, function(resp) { console.log(resp); });
}

// Check links and add onclick to BeFrank URIs:
function checkLinksOnPage() {
  var links_on_page = document.getElementsByTagName("a");
  for (var i = 0; i < links_on_page.length; i++) {
    if (links_on_page[i].href.substring(0,7) == 'befrank:') {
      links_on_page[i].onclick = function () { openBeFrankWalletCLIsend(this.href); return false; }
    }
  }
}
