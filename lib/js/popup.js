...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('auth-form').addEventListener('submit', function (e) {
    ...
    var request = {
      greeting: "BeFrank befrank-wallet-rpc Update Wallet Info",
...
var get_wallet_info = function () {
  var request = {greeting: "BeFrank befrank-wallet-rpc Send Wallet Info"};
  chrome.runtime.sendMessage(request, function (resp) {
    ...
    if (resp.status == 'ok') {
      ...
      if (wallet_info.address != resp.address) {
        ...
        var qrcode = new QRCode("address-qr", {
          text: "befrank:" + wallet_info.address,
          width: 192,
          height: 192,
          colorDark : "#000000",
          colorLight : "#ffffff",
  ...
  makeIntegratedAddress(wallet_info.port, payment_id,
    function (resp) {
      ...
      if (resp.hasOwnProperty('result')) {
        ...
        var uri_string = 'befrank:' + integrated_address;
...
function receiveNormalQR (payment_id, amount) {
  ...
  var uri_string = 'befrank:' + wallet_info.address;
  if (amnt_str.length > 0)   uri_string += '?amount=' + amnt_str;
  ...
  // contactsDB.open(function () {
    contactsDB.fetchContacts(function(contacts) {
      ...
      for (var i = 0; i < contacts.length; i++) {
        ...
        contactName.setAttribute("data-id", contact.bfr_address);
...
function storeNewContact () {
  ...
  var bfr_addr = contactAddr.value;
...
function confirmSend () {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if (amount != '') amnt.innerHTML = amount + ' BFR';
    li.appendChild(amnt);
...
// Send BeFrank to destination on button click
function sendBeFrank () {
  var addresses = document.getElementsByClassName('send-input-dest');
  var amounts = document.getElementsByClassName('send-input-amount');
  var dests = [];
  for (var i=0; i < addresses.length; i++) {
  ...
  transferSplit(wallet_info.port, dests, pay_id, fee, mixin, unlock_time, get_tx_key, new_algo,
    ...
    function (err) {
      var status = document.getElementById('send-error');
      status.innerHTML = 'There was an error connecting to befrank-wallet-rpc.';
      status.style.display = 'block';
      setTimeout(function() {
        status.style.display = 'none';
      }, 10000);
