...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  ...
  if (!sender.tab ||
    ...
    if (request.greeting == "BeFrank befrank-wallet-rpc Send Wallet Info") {
      sendResponse(wallet_info);
    } else if (request.greeting == "BeFrank befrank-wallet-rpc Update Wallet Info") {
  ...
  if (Number(wallet_info.port) >= 1 && Number(wallet_info.port) <= 65535 && wallet_info.status == 'ok') {
    getBalance(wallet_info.port,
      function (resp) {
        ...
        wallet_info.balance = coinsFromAtomic(resp.result.available_balance.toString());
        wallet_info.unlockedBalance = coinsFromAtomic(resp.result.locked_amount.toString());
