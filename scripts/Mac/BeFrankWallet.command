...
# https://github.com/befrank-project/befrank-wallet-chrome
#
...
echo "===================================================================================="
echo "                    BeFrank befrank-wallet-cli Launcher for OS X"
echo ""
echo "Copyright (c) 2016 bigreddmachine."
echo "Released under the MIT License as part of BeFrank Wallet for Chrome."
echo "https://github.com/befrank-project/befrank-wallet-chrome"
echo "===================================================================================="
echo ""

mkdir -p ~/Library/Application\ Support/BeFrankWalletChrome

sleep 1

cd ~/Library/Application\ Support/BeFrankWalletChrome
FILE="wallet.bin.wallet"
FILE_NO_EXTENSION="wallet.bin"

echo "Which BeFrank daemon would you like to connect to?"
echo "  1. remote (blocks.befrank.site:18322)"
echo "  2. local  (127.0.0.1:18322)"
echo "  3. custom"
echo ""
...
if [ $DAEMON -eq 3 ]; then
    echo "Please enter custom daemon in quotes (example: \"192.168.0.1:18322\"):"
    read CUSTOM_DAEMON
fi
...
if [ -f "$FILE" ]; then
    ...
    # if [ $DAEMON -eq 1 ]; then
    #     screen -S BeFrankWalletCLIrpc -d -m /Applications/BeFrankWallet/befrank-wallet-cli --wallet-file $FILE --password $PASS --rpc-bind-ip 127.0.0.1 --rpc-bind-port 18324 --daemon-address blocks.befrank.site:18322
    # elif [ $DAEMON -eq 2 ]; then
    #     screen -S BeFrankWalletCLIrpc -d -m /Applications/BeFrankWallet/befrank-wallet-cli --wallet-file $FILE --password $PASS --rpc-bind-ip 127.0.0.1 --rpc-bind-port 18324
    # else
    #     screen -S BeFrankWalletCLIrpc -d -m /Applications/BeFrankWallet/befrank-wallet-cli --wallet-file $FILE --password $PASS --rpc-bind-ip 127.0.0.1 --rpc-bind-port 18324 --daemon-address $CUSTOM_DAEMON
    # fi

    if [ $DAEMON -eq 1 ]; then
        screen -S BeFrankWalletCLIrpc -d -m /Applications/BeFrankWallet/befrank-wallet-cli --wallet-file $FILE_NO_EXTENSION --password $PASS --rpc-bind-ip 127.0.0.1 --rpc-user me --rpc-password awesome --rpc-bind-port 18324 --daemon-address blocks.befrank.site:18322
    elif [ $DAEMON -eq 2 ]; then
        screen -S BeFrankWalletCLIrpc -d -m /Applications/BeFrankWallet/befrank-wallet-cli --wallet-file $FILE_NO_EXTENSION --password $PASS --rpc-bind-ip 127.0.0.1 --rpc-user me --rpc-password awesome  --rpc-bind-port 18324 --daemon-address 127.0.0.1:18322
    else
        screen -S BeFrankWalletCLIrpc -d -m /Applications/BeFrankWallet/befrank-wallet-cli --wallet-file $FILE_NO_EXTENSION --password $PASS --rpc-bind-ip 127.0.0.1 --rpc-user me --rpc-password awesome --rpc-bind-port 18324 --daemon-address $CUSTOM_DAEMON
    fi
    ...
    if [ $DAEMON -eq 1 ]; then
        /Applications/BeFrankWallet/befrank-wallet-cli --generate-new-wallet $FILE --password $NEWPASS --daemon-address blocks.befrank.site:18322
    elif [ $DAEMON -eq 2 ]; then
        /Applications/BeFrankWallet/befrank-wallet-cli --generate-new-wallet $FILE --password $NEWPASS
    else
        /Applications/BeFrankWallet/befrank-wallet-cli --generate-new-wallet $FILE --password $NEWPASS --daemon-address $CUSTOM_DAEMON
    fi
