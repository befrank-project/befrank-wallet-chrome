...
function save_options() {
  ...
  var walletPort = String(document.getElementById('befrank-wallet-rpc-port').value);
  ...
  if (Number(walletPort) >= 1 && Number(walletPort) <= 65535) {
    ...
    chrome.storage.sync.set({
      ...
      var status = document.getElementById('status');
      updateWalletInfo(walletPort, saveAuth, username, password);
      document.getElementById('befrank-wallet-rpc-port').value = 18324;
      status.textContent = 'Options saved. Port invalid... set to 18324.';
      setTimeout(function() {
        status.textContent = '';
      }, 2000);
    });
...
function restore_options() {
  ...
  chrome.storage.sync.get({
    ...
    document.getElementById('befrank-wallet-rpc-port').value = items.walletPort;
...
function clear_storage() {
  ...
  chrome.storage.sync.clear( function() {
    var status = document.getElementById('status')
    document.getElementById('befrank-wallet-rpc-port').value = '';
